...
// <api-imports>
import gallery_get_0 from "./api/gallery/GET";
import gallery_post_1 from "./api/gallery/POST";
import gallery_upload_post_2 from "./api/gallery/upload/POST";
import gallery_upload_form_post_3 from "./api/gallery/upload-form/POST";
import health_get_4 from "./api/health/GET";
import maps_config_get_5 from "./api/maps/config/GET";
// </api-imports>
...
// =========================================================
// PASTE THIS DIRECTLY UNDER EXPRES.URLENCODED:
// =========================================================
...
export function registerAdSenseTextRoutes(
  targetApp: import("express").Express,
  config: AdSenseConfig,
) {
  targetApp.get("/ads.txt", (_req, res) => {
    res.type("text/plain").set("Cache-Control", "no-cache");
    if (config.adsTxt) res.status(200).send(config.adsTxt);
    else res.status(404).send("");
  });
  targetApp.get("/app-ads.txt", (_req, res) => {
    res.type("text/plain").set("Cache-Control", "no-cache");
    if (config.appAdsTxt) res.status(200).send(config.appAdsTxt);
    else res.status(404).send("");
  });
}

type SsrResult = { head: string; html: string };
type SsrAdSense = { scriptHtml: string };
...
export function renderSsrDocument(
  ...
  const headContent = result.head + (adsense.scriptHtml ? "\n" + adsense.scriptHtml : "");
  return template
