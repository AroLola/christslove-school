...
type AlbumSection = { id: string; name: string; description?: string; media: MediaItem[] }; 

export default function GalleryPage() { 
  const title = "Gallery — Christ's Love Christian School"; 
  const description = "Browse school event photo albums from Christ's Love Christian School."; 
  const canonicalUrl = `${site}/gallery`; 
  
  const [albums, setAlbums] = useState<AlbumSection[]>([]); 
  ...
  useEffect(() => { 
    fetch('/gallery-data.json')
      ...
      .catch((err) => console.error("Error loading gallery asset streams:", err));
  }, []); 
      ...
      <div className="py-16 bg-background min-h-screen"> 
        <div className="container mx-auto px-4 lg:px-8"> 
          ...
          <div className="text-center max-w-2xl mx-auto mb-20"> 
            <h1 className="font-heading text-4xl md:text-5xl font-bold text-secondary mb-4">School Media Gallery</h1> 
            <p className="text-muted-foreground text-lg">Capturing moments of faith, learning, and victory at CLCS.</p> 
          </div> 
          
          {/* Loop over each of the 6 Album Sections */} 
          <div className="space-y-24">
            {albums.map((album) => (
              <section key={album.id} className="border-b border-secondary-foreground/5 pb-16 last:border-0 last:pb-0">
                
                {/* Album Header & Section Description Caption */}
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6 w-full"> 
                  {album.media?.map((img) => ( 
                    ...
                    > 
                      ...
                      {img.caption && (
                        <div className="mt-3"> 
                          <p className="text-sm font-medium text-secondary/80 group-hover:text-primary transition-colors">
                            {img.caption}
                          </p> 
                        </div> 
                      )}
