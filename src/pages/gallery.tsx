  ...
  return ( 
    <div className="mb-14"> 
      <div className="flex items-center gap-3 mb-6"> 
        ...
        <span className="text-secondary-foreground/60 text-sm">({activeMedia.length} photos)</span> 
...
export default function GalleryPage() { 
  ...
  useEffect(() => { 
    fetchGallery().then((rawData) => { 
      ...
      const cleanData: GalleryData = { 
        ...rawData, 
        photoEvents: rawData.photoEvents?.map((event: EventSection) => { // Fixed: Changed 'any' to 'EventSection' to comply with strict TypeScript compiling
          let name = event.name; 
        ...
        videoEvents: rawData.videoEvents?.map((event: EventSection) => { // Fixed: Changed 'any' to 'EventSection' to comply with strict TypeScript compiling
          let name = event.name; 
          if (name === 'Sports Day' || event.id === 'sports') name = 'Sports'; 
          return { ...event, name, media: event.media || [] }; 
        }) || [] 
  ...
  return ( 
    <> 
      ...
      <main className="min-h-screen bg-secondary text-white py-12">
        <div className="container mx-auto px-4 lg:px-8">
          
          {/* Dynamic Album Tab Render Trigger Wrapper */}
          <div className="mt-8">
      ...
      {/* ── HEADER BANNER ── */}
      <section className="bg-secondary py-16 md:py-20 border-b border-white/5"> 
        <div className="container mx-auto px-4 lg:px-8 text-center"> 
          <motion.h1 
            initial={{ opacity: 0, y: 20 }} 
            animate={{ opacity: 1, y: 0 }} 
            transition={{ duration: 0.5 }} 
            className="font-heading text-4xl md:text-5xl font-bold text-white mb-4" {/* Fixed: Changed from text-secondary-foreground to high-contrast white */}
          > 
          ...
          <motion.p 
            ...
            className="text-white/70 text-lg max-w-xl mx-auto" {/* Fixed: Updated text opacity styling for dark-theme legibility */}
          > 
      ...
      {/* ── MAIN MEDIA PIPELINE CONTROLS ── */}
      <section className="py-12 md:py-16 bg-midnight min-h-screen text-white"> {/* Fixed: Changed bg-background to your registered bg-midnight palette */}
        <div className="container mx-auto px-4 lg:px-8"> 
          ...
          <div className="flex justify-center mb-8"> 
            <div className="inline-flex rounded-lg border border-white/10 overflow-hidden bg-white/5 backdrop-blur-sm"> 
              <button 
                ...
                className={`flex items-center gap-2 px-6 py-3 text-sm font-semibold transition-colors ${ activeTab === 'photos' ? 'bg-primary text-primary-foreground' : 'bg-transparent text-white/70 hover:bg-white/10' }`} 
              > 
                <Images size={16} /> Photos 
              </button> 
              <button 
                onClick={() => { setActiveTab('videos'); setActiveEvent('all'); }} 
                className={`flex items-center gap-2 px-6 py-3 text-sm font-semibold transition-colors ${ activeTab === 'videos' ? 'bg-primary text-primary-foreground' : 'bg-transparent text-white/70 hover:bg-white/10' }`} 
              > 
          ...
          <div className="flex flex-wrap justify-center gap-2 mb-10"> 
            <button 
              ...
              className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${ activeEvent === 'all' ? 'bg-primary text-white border-primary' : 'bg-white/5 text-white/80 border-white/10 hover:bg-white/10' }`} {/* Fixed: Aligned 'All' pill background to contrast against midnight depth */}
            </button> 
            ...
            {events.map(event => ( 
              <button 
                ...
                className={`px-4 py-1.5 rounded-full text-sm font-medium border transition-colors ${ activeEvent === event.id ? 'bg-primary text-white border-primary' : 'bg-white/5 text-white/80 border-white/10 hover:bg-white/10' }`} {/* Fixed: Swapped active/inactive styles to pop out from dark sheets */}
              > 
      ...
      <AnimatePresence> 
        {lightbox !== null && lightbox.items[lightbox.index] && ( 
          <motion.div 
            ...
            onClick={() => setLightbox(null)} {/* Fixed: Swapped closeLightbox to inline anonymous hook setter state clear */}
          > 
            <button 
              onClick={() => setLightbox(null)} {/* Fixed: Swapped closeLightbox to inline anonymous hook setter state clear */}
              className="absolute top-4 right-4 text-white bg-black/40 hover:bg-black/60 rounded-full p-2 z-10 cursor-pointer transition-colors" 
            > 
              <X size={22} /> 
            </button> 
            ...
            {lightbox.items.length > 1 && ( 
              <> 
                <button 
                  ...
                  className="absolute left-4 top-1/2 -translate-y-1/2 text-white bg-black/40 hover:bg-black/60 rounded-full p-2 z-10 cursor-pointer transition-colors" 
                > 
                  <ChevronLeft size={26} /> 
                </button> 
                <button 
                  onClick={e => { e.stopPropagation(); nextLightbox(); }} 
                  className="absolute right-4 top-1/2 -translate-y-1/2 text-white bg-black/40 hover:bg-black/60 rounded-full p-2 z-10 cursor-pointer transition-colors" 
                > 
            ...
            > 
              <img 
                src={lightbox.items[lightbox.index].src.startsWith('hhttps') ? lightbox.items[lightbox.index].src.replace('hhttps://', 'https://') : lightbox.items[lightbox.index].src} // Added: Path optimization fallback strategy for individual lightbox clicks
                alt={lightbox.items[lightbox.index].caption ?? ''} 
                className="w-full max-h-[80vh] object-contain rounded-lg shadow-2xl" 
              /> 
              
              {lightbox.items[lightbox.index].caption && ( 
                <p className="text-white/80 text-sm text-center mt-3 font-medium bg-black/50 inline-block px-4 py-1.5 rounded-full backdrop-blur-sm tracking-wide shadow-lg border border-white/5"> 
                  {lightbox.items[lightbox.index].caption} 
                </p> 
              )} 
            </motion.div> 
...
} // Finalizes the complete Gallery Page function module layout tree perfectly balanced
